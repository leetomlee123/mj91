...
class _CustomFijkPanelState extends State<CustomFijkPanel>
  ...
  var find = Get.find<MoviePlayerController>();

  ShowConfigAbs get showConfig => widget.showConfig;

  List<Items> get _videos => widget.videos!;
...
find.savePosition();
  ...
  List<Widget> _createTabConList() {
    List<Widget> wds = [];
    for (var activeIdx = 0; activeIdx < _videos.length; activeIdx++) {
      wds.add(Padding(
        ...
        child: ElevatedButton(
          style: ButtonStyle(
            ...
            backgroundColor: MaterialStateProperty.all(
                activeIdx == widget.curActiveIdx ? Colors.red : Colors.blue),
          ),
          onPressed: () {
            int newActiveIdx = activeIdx;
            // 切换播放源
            changeCurPlayVideo(newActiveIdx);
          },
          child: Text(
            _videos[activeIdx].name!.trim(),
            style: TextStyle(
              color: Colors.white,
            ),
          ),
      ...
      child: Column(
        ...
        children: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              // 切换视频
              changeCurPlayVideo(widget.curActiveIdx);
            },
            child: Text(
              "点击重试",
  ...
  Widget _buildIdleStateSlotWidget() {
    return IconButton(
      ...
      onPressed: () async {
        int newActiveIdx = widget.curActiveIdx ?? 0;
        await changeCurPlayVideo(newActiveIdx);
      },
    );
  }
...
class _buildGestureDetectorState extends State<_buildGestureDetector> {
  ...
  List<Items> get _videoSourceTabs => widget.videoFormat!;
  ...
  void _playerValueChanged() async {
    ...
    bool isOverFlow = widget.curActiveIdx + 1 >= _videoSourceTabs.length;
  ...
  Widget _buildTopBar() {
    return AnimatedOpacity(
      opacity: _hideStuff ? 0.0 : 0.8,
      duration: Duration(milliseconds: 400),
      child: showConfig.topBar && _playing
          ? Container(
              height: showConfig.stateAuto && !widget.player.value.fullScreen
                  ? 35
                  : barHeight,
              ...
              child: Container(
                alignment: Alignment.topLeft,
                height: barHeight - (widget.player.value.fullScreen ? 0 : 20),
                child: Row(
                  children: <Widget>[
                    _buildTopBackBtn(),
                    Expanded(
